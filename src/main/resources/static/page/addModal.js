(function() {
    ...
    if (window.addModalInitialized) return;
    window.addModalInitialized = true;
    var addModal={
        mediaRecorder: null,
        audioChunks: [],
        ...
        init: function() {
            $('.start-record-icon').off('click').on('click', (e) => {
                let field = $(e.currentTarget).attr('data-target');
                this.startRecording(field);
                $(e.currentTarget).hide();
                $(`[data-target="${field}"].stop-record-icon`).show();
            });

            $('.stop-record-icon').off('click').on('click', (e) => {
                let field = $(e.currentTarget).attr('data-target');
                this.stopRecording();
                $(e.currentTarget).hide();
                $(`[data-target="${field}"].start-record-icon`).show();
            });
        }
    };
