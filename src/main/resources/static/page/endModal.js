(function() {
    // Prevent double initialization of endModal
    if (window.endModalInitialized) return;
    window.endModalInitialized = true;
    var endModal = {
        mediaRecorder: null,
        audioChunks: [],
        currentRecordingField: null,
        radioOptionsMappings: {
            'completeCameraOption': {
                '정상': '정상',
                '곰팡이': '곰팡이',
                '결로': '결로',
                '단열재 누락': '단열재 누락',
        ...
        preprocessTranscription: function(transcription, field) {
            ...
            switch (field) {
                case 'completeLadonRoomName':
                case 'completeFormaldehydeRoomName':
                case 'completeCameraRoomName':
                case 'completePipeRoomName':
                case 'completeBathRoomName':
                case 'completeFinalRoomName':
                    return this.convertSpokenRoomNameToDropdownValue(transcription);
                case 'completeLadonPciInput':
                case 'completeFormaldehydePpmInput':
        ...
        init: function() {
            $('.estart-record-icon').off('click').on('click', (e) => {
                let field = $(e.currentTarget).attr('data-target');
                this.startRecording(field);
                $(e.currentTarget).hide();
                $(`[data-target="${field}"].estop-record-icon`).show();
            });

            $('.estop-record-icon').off('click').on('click', (e) => {
                let field = $(e.currentTarget).attr('data-target');
                this.stopRecording();
                $(e.currentTarget).hide();
                $(`[data-target="${field}"].estart-record-icon`).show();
            });
        }
    };
